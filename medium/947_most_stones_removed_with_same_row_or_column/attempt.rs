...
impl Solution
{
    pub fn remove_stones(stones: Vec<Vec<i32>>) -> i32
    {
        ...
        {
            if x != *parent.entry(x).or_insert(x)
            {
                let px = *parent.get(&x).unwrap();
                let root = find(px, parent);
                parent.insert(x, root);
            }
            *parent.get(&x).unwrap()
        }

        fn union(x: i32, y: i32, parent: &mut HashMap<i32, i32>)
        {
        ...
        let keys: Vec<i32> = parent.keys().cloned().collect();
        for x in keys {
            unique_parents.insert(find(x, &mut parent));
        }

        (stones.len() - unique_parents.len()) as i32
    }
}